...
/**
 * Test: testCreateTwoApigateways
 * Validates creating multiple API Gateways and connecting endpoints
 */
export async function testCreateTwoApigateways() {
    const testName = 'testCreateTwoApigateways';
    ...
    try {
        ...
        // Step 2: Add api2 without endpoint
        console.log(chalk.gray('  Adding api2 (no endpoint)...'));
        await createApiGatewayProgrammatically(outputPath, {
            gatewayName: 'api2'
        });

        results.push({ step: 'api2 created', passed: true });

        // Step 3: Add api3 with POST /test endpoint
        console.log(chalk.gray('  Adding api3 with POST /test...'));
        await createApiGatewayProgrammatically(outputPath, {
            gatewayName: 'api3',
            endpoint: {
                method: 'post',
        ...
        results.push({ step: 'api3 created with endpoint', passed: true });
